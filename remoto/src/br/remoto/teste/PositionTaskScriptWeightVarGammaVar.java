...
public class PositionTaskScriptWeightVarGammaVar 
{
	...
	public PositionTaskScriptWeightVarGammaVar()
	{
		// Specify ReMoto's path
		
		
		ReMoto.path = "C:\\Users\\renato\\Dropbox\\PositionTask\\TPweightAndGammaStatic\\";
	...
	{
		PositionTaskScriptWeightVarGammaVar teste = new PositionTaskScriptWeightVarGammaVar();
	...
	public void run() throws FileNotFoundException
	{
		...
		for(int j=1; j <= numOfGammaVar; j++){
			for(int k = 1; k <= numOfSimulations; k++){
			
				//conf.setPrimaryBag2AndChainGain(3500 + 100 * (k - 1));
				conf.setpercentageOfMVC(0.1*k);
				conf.setGammaStatic(30 + (j - 1) * 2);
				
				if(k==1){
					System.out.println("Mean = 10.8" + " Weight= " + conf.getpercentageOfMVC());
				...
				if(k==2){
					System.out.println("Mean = 9" + " Weight= " + conf.getpercentageOfMVC());
					//conf.setGammaDynamic(32);
				...
				if(k==3){
					System.out.println("Mean = 8" + " Weight= " + conf.getpercentageOfMVC());
					//conf.setGammaDynamic(34);
					//conf.setGammaStatic(34);
					for(int i = 0; i < conf.getNeuronTypes(ReMoto.DT, ReMoto.TR).size(); i++){
				...
				if(k==4){
					System.out.println("Mean = 7.2"  + " Weight= " + conf.getpercentageOfMVC());
					for(int i = 0; i < conf.getNeuronTypes(ReMoto.DT, ReMoto.TR).size(); i++){
				...
				if(k==5){
					System.out.println("Mean = 6.4" + " Weight= " + conf.getpercentageOfMVC());
					for(int i = 0; i < conf.getNeuronTypes(ReMoto.DT, ReMoto.TR).size(); i++){
				...
				if(k==6){
					System.out.println("Mean = 6" + " Weight= " + conf.getpercentageOfMVC());
					for(int i = 0; i < conf.getNeuronTypes(ReMoto.DT, ReMoto.TR).size(); i++){
				...
				if(k==7){
					System.out.println("Mean = 5.4" + " Weight= " + conf.getpercentageOfMVC());
					for(int i = 0; i < conf.getNeuronTypes(ReMoto.DT, ReMoto.TR).size(); i++){
				...
				if(k==8){
					System.out.println("Mean = 5" + " Weight= " + conf.getpercentageOfMVC());
					for(int i = 0; i < conf.getNeuronTypes(ReMoto.DT, ReMoto.TR).size(); i++){
				...
				System.out.println(" Weight= " + conf.getpercentageOfMVC());
				...
				generateAndStoreData("muscleLength", sim, dataPath, graphicPath, path, k, j);
	...
	public void generateAndStoreData(String output, Simulation sim, String dataPath, String graphicPath, String path, int simCount1, int simCount2) throws FileNotFoundException{
		...
		int varStep = 10 * simCount1;
		int varGamma = 30 + 2 * (simCount2 - 1);
	
		
		outputFile = new File(dataPath + path + "\\" + output + "_weight" + varStep + "_gammaStatic" + varGamma + ".txt");
	    ...
	    if(output.equals("spikesMNs") || output.equals("spikesIas") || output.equals("firingRateMNs") || output.equals("spikesIaINs") ){
	    	PlotScatter.generate(datasetS,
					 graphicPath + path + "\\" + output + "_weight" + varStep + "_gammaStatic" + varGamma + ".jpg",
					 output, 
					 "Time [ms]", 
					 output);
	    }
	...
	public void generateLogOfScenario(int simCount, String dataPath, String path) throws FileNotFoundException{
		...
		output = new File(dataPath + path + "\\scenario" + "_weight" + simCount + ".txt");
