...
public class PoolIdentificationSOLMUvarScript 
{
	...
	public PoolIdentificationSOLMUvarScript()
	{
		// Specify ReMoto's path
		
		
		ReMoto.path = "/home/renato/Documents/Doutorado/Semestre2_2013/MNPoolIdentification/";
	...
	{
		PoolIdentificationSOLMUvarScript teste = new PoolIdentificationSOLMUvarScript();
	...
	public void run() throws FileNotFoundException
	{
		...
		conf = simDAO.getConfiguration(34);
	...
	for (int j = 3; j <=3; j=j+1){
		for(int k = 23; k <= numOfSimulations; k++){
			
			conf.setQuantityNeuronType("SOL", "S", 400 + (k - 1) * 50);
	...
	public void generateAndStoreData(String output, Simulation sim, String dataPath, String graphicPath, String path, int simCount, int simCount2) throws FileNotFoundException{
		...
		outputFile = new File(dataPath + path + "/" + output + "_MUvar_" + (int)(400 + 50*(simCount-1)) + "p" + simCount2 + ".txt");
	    ...
	    if(output.equals("spikesMNs") || output.equals("spikesIas") || output.equals("firingRateMNs") || output.equals("spikesIaINs") ){
	    	PlotScatter.generate(datasetS,
					 graphicPath + path + "/" +  output + "_MUVar_" + (int)((400 + 50*(simCount-1))) + "p" + simCount2 + ".jpg",
					 output, 
					 "Time [ms]", 
					 output);
	    }
	    	...
	    	PlotXYLine.generate(datasetS,
	    			 graphicPath + path + "/"  + output + "_MUVar_" + (400 + 50*(simCount-1)) + "p" + simCount2 + ".jpg",
					 output, 
					 "Time [ms]", 
					 output);
	    }
	...
	public void generateLogOfScenario(int simCount, String dataPath, String path, int simCount2) throws FileNotFoundException{
		...
		output = new File(dataPath + path + "/scenario" + (int)((400 + 50*(simCount-1))) + "p" + simCount2 + ".txt");
