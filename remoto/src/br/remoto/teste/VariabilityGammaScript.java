...
public class VariabilityGammaScript 
{
	...
	public VariabilityGammaScript()
	{
		// Specify ReMoto's path
		
		
		ReMoto.path = "/home/renato/Dropbox/Doutorado/Variability/";
	...
	{
		VariabilityGammaScript teste = new VariabilityGammaScript();
	...
	public void run() throws FileNotFoundException
	{
		...
		conf = simDAO.getConfiguration(32);
		...
		*/
		...
		for(int k = 4; k <= 12; k++){
			...
			generateAndStoreData("muscleForceSOL", sim, dataPath, graphicPath, path, k, m, j);
			generateAndStoreData("muscleForceMG", sim, dataPath, graphicPath, path, k,m, j);
			generateAndStoreData("muscleForceLG", sim, dataPath, graphicPath, path, k,m, j);
			...
			generateAndStoreData("jointTorque", sim, dataPath, graphicPath, path, k,m, j);
			//generateAndStoreData("jointDisturbance", sim, dataPath, graphicPath, path, k);
			generateAndStoreData("conductanceSOL", sim, dataPath, graphicPath, path, k,m, j);
			generateAndStoreData("conductanceMG", sim, dataPath, graphicPath, path, k,m, j);
			
			System.out.println("-> Simulation - Order " + k + "Mean" + m + "Trial" + j + " end");
		}
		}
		}
	...
	public void generateAndStoreData(String output, Simulation sim, String dataPath, String graphicPath, String path, int simCount, int simCount3, int simCount2) throws FileNotFoundException{
		...
		outputFile = new File(dataPath + path + "/" + output + "_order_" + simCount3 + "_mean_"+ simCount + "_trial_" + simCount2 + ".txt");
	    ...
	    if(output.equals("spikesMNs") || output.equals("spikesIas") || output.equals("firingRateMNs") || output.equals("spikesIaINs") ){
	    	PlotScatter.generate(datasetS,
					 graphicPath + path + "/" +  output + "_order_" + simCount3 + "_mean_"+ simCount + "_trial_" + simCount2 + ".jpg",
					 output, 
					 "Time [ms]", 
					 output);
	    }
