...
public class ConductanceEstimationScript 
{
	...
	public ConductanceEstimationScript()
	{
		// Specify ReMoto's path
		
		
		ReMoto.path = "/home/renato/Dropbox/Doutorado/Conductance/";
	...
	{
		ConductanceEstimationScript teste = new ConductanceEstimationScript();
	...
	public void run() throws FileNotFoundException
	{
		...
		conf = simDAO.getConfiguration(33);
	...
	for (int l = 1; l <= 10; l++){
	   for(int k = 41; k <= 41; k++){
		for (int j = 1; j <= 1; j= j + 1){
			...
			generateLogOfScenario(k, dataPath, path, j, l);
			...
			generateAndStoreData("conductanceSOL", sim, dataPath, graphicPath, path, k, j, l);
			generateAndStoreData("conductanceMG", sim, dataPath, graphicPath, path, k, j, l);
	...
	public void generateAndStoreData(String output, Simulation sim, String dataPath, String graphicPath, String path, int simCount, int simCount2, int simCount3) throws FileNotFoundException{
		...
		outputFile = new File(dataPath + path + "/" + output + "ISI" + simCount + "Order" + simCount3 + "Trial" + simCount2 + ".txt");
	    ...
	    if(output.equals("spikesMNs") || output.equals("spikesIas") || output.equals("firingRateMNs") || output.equals("spikesIaINs") ){
	    	PlotScatter.generate(datasetS,
					 graphicPath + path + "/" +  output + "ISI" + simCount + "Order" + simCount3 + "Trial" + simCount2 + ".jpg",
					 output, 
					 "Time [ms]", 
					 output);
	    }
	...
	public void generateLogOfScenario(int simCount, String dataPath, String path, int simCount2, int simCount3) throws FileNotFoundException{
		...
		output = new File(dataPath + path + "/scenario" + "ISI" + simCount + "Order" + simCount3 +  "Trial" + simCount2 + ".txt");
