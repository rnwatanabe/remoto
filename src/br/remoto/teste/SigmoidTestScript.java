...
public class SigmoidTestScript 
{
	...
	public SigmoidTestScript()
	{
		// Specify ReMoto's path
		
		
		ReMoto.path = "/home/renato/Dropbox/papermestrado/SupplementalData/SigmoidRaikovaNew/";
	...
	{
		SigmoidTestScript teste = new SigmoidTestScript();
	...
	public void run() throws FileNotFoundException
	{
		...
		conf = simDAO.getConfiguration(29);
		...
		*/
		...
		for(int k = 1; k <= numOfSimulations; k++){
			...
			System.out.println(nerves.size());
			
			for(int y = 0; y < nerves.size(); y++)
			{
				NerveVO nerve = (NerveVO)nerves.get(y);	
		    	//NerveVO nerve = (NerveVO)conf.getNerveStimulations().get(y);
			...
			//generateAndStoreData("muscleForceS1", sim, dataPath, graphicPath, path, k);
			generateAndStoreData("muscleForceS5", sim, dataPath, graphicPath, path, k);
			generateAndStoreData("muscleForceFR3", sim, dataPath, graphicPath, path, k);
			generateAndStoreData("muscleForceFF3", sim, dataPath, graphicPath, path, k);
	...
	public void generateAndStoreData(String output, Simulation sim, String dataPath, String graphicPath, String path, int simCount) throws FileNotFoundException{
		...
		double varStep = 0.1 + (simCount - 1) * 1.6;
	
		
		outputFile = new File(dataPath + path + "/" + output + (int)(10*varStep) + "Hz" + ".txt");
	    ...
	    if(output.equals("spikesMNs") || output.equals("spikesIas") || output.equals("firingRateMNs") || output.equals("spikesIaINs") ){
	    	PlotScatter.generate(datasetS,
					 graphicPath + path + "/" +  output + (int)(10*varStep) + "Hz" + ".jpg",
					 output, 
					 "Time [ms]", 
					 output);
	    }
	...
	public void generateLogOfScenario(int simCount, String dataPath, String path) throws FileNotFoundException{
		...
		output = new File(dataPath + path + "/scenario" +  (int)(10*varStep) + "Hz" + ".txt");
