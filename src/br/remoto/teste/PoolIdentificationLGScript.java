...
public class PoolIdentificationLGScript 
{
	...
	public PoolIdentificationLGScript()
	{
	...
	{
		PoolIdentificationLGScript teste = new PoolIdentificationLGScript();
	...
	public void run() throws FileNotFoundException
	{
		...
		conf = simDAO.getConfiguration(35);
	...
	for (int j = 10; j <= 10; j=j+1){
		for(int k = 7; k <= numOfSimulations; k++){
			...
			generateAndStoreData("conductanceLG", sim, dataPath, graphicPath, path, k, j);
	...
	public void generateAndStoreData(String output, Simulation sim, String dataPath, String graphicPath, String path, int simCount, int simCount2) throws FileNotFoundException{
		...
		else if(output.equals("conductanceLG")){
			nmSubplots[0].add(			"conductance");
			nmCdNeurons[0].add(			"MN S 1");
			nmCdSpecification[0].add(	"gExc-dend");
			yLabels[0].add(				"Conductance LG");
			legendLabels[0].add(		"LG");
			nmMuscles[0].add(			"MG");
		}
		...
		outputFile = new File(dataPath + path + "/" + output + "_Gamma_" + (int)(10*simCount) + "p" + simCount2 + ".txt");
	    ...
	    if(output.equals("spikesMNs") || output.equals("spikesIas") || output.equals("firingRateMNs") || output.equals("spikesIaINs") ){
	    	PlotScatter.generate(datasetS,
					 graphicPath + path + "/" +  output + "_Gamma_" + (int)(10*simCount) + "p" + simCount2 + ".jpg",
					 output, 
					 "Time [ms]", 
					 output);
	    }
	    	...
	    	PlotXYLine.generate(datasetS,
	    			 graphicPath + path + "/"  + output + "_Gamma_" + 10*simCount + "p" + simCount2 + ".jpg",
					 output, 
					 "Time [ms]", 
					 output);
	    }
