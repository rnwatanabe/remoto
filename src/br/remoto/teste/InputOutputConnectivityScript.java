...
public class InputOutputConnectivityScript 
{
	...
	public InputOutputConnectivityScript()
	{
		// Specify ReMoto's path
		
		
		ReMoto.path = "/home/renato/Dropbox/Doutorado/Connectivity/";
	...
	{
		InputOutputConnectivityScript teste = new InputOutputConnectivityScript();
	...
	for (int j = 1; j <= 3; j=j+1){
		for(int k = 5; k <= 100; k=k+5){
			...
			//conf.setQuantityNeuronType("DT", ReMoto.CM_ext, 12000 / k);
		    ...
		    for(int i = 0; i < conductances.size(); i++){
		    	...
		    	if (g.getCdNucleus().equals("SOL")){
		    		g.setConnectivity(k);
		    	}
		    	if (g.getCdNucleus().equals("MG")){
		    		g.setConnectivity(k);
		    	}
	...
	public void generateAndStoreData(String output, Simulation sim, String dataPath, String graphicPath, String path, int simCount, int simCount2) throws FileNotFoundException{
		...
		outputFile = new File(dataPath + path + "/" + output + "_connectivity_"+(int)(simCount) + "_" + simCount2 + "_ISI.txt");
	    ...
	    if(output.equals("spikesMNs") || output.equals("spikesIas") || output.equals("firingRateMNs") || output.equals("spikesIaINs") ){
	    	PlotScatter.generate(datasetS,
					 graphicPath + path + "/" +  output + "_connectivity_"+(int)(simCount) + "_" + simCount2 + "_ISI.jpg",
					 output, 
					 "Time [ms]", 
					 output);
	    }
	...
	public void generateLogOfScenario(int simCount, String dataPath, String path, int simCount2) throws FileNotFoundException{
		...
		output = new File(dataPath + path + "/scenario" + "_connectivity_"+(int)(simCount) + "_" + simCount2 + "_ISI.txt");
	    ...
	    for(int i = 0; i < conductances.size(); i++){
	    	...
	    	printWriter.println("g.getCdConductanceType(): " + g.getCdConductanceType() + "\t" +
	    						"g.getGmax(): " + g.getGmax() + "\t" + 
	    						"g.getCdNucleus(): " + g.getCdNucleus() + "\t" +
	    						"g.getConnectivity(): " + g.getConnectivity());
